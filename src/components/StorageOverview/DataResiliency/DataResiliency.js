...
export const DataResiliency = ({ stalePgRaw, totalPgRaw, cleanPgRaw, LoadingComponent }) => {
  const staleg = getCapacityStats(stalePgRaw);
  const totalPg = getCapacityStats(totalPgRaw);
  const cleanPg = getCapacityStats(cleanPgRaw);
  //const progressPercentage = (((cleanPg - stalePg) / totalPg) * 100).toFixed(1);
  ...
  return (
    <DashboardCard>
      ...
      >
        {progressPercentage % 100 === 0 || !(stalePg && totalPg && cleanPg) ? (
          <DataResiliencyStatusBody isStatusSuccessful={stalePg && totalPg && cleanPg} />
        ) : (
